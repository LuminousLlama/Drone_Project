...
//                ToastUtils.setResultToToast(flightController.getRollPitchControlMode().name());
                ...
                VirtualStickDataManual task = new VirtualStickDataManual(0,0,90,0);

                VirtualStickDataManual task1 = new VirtualStickDataManual(0,0, 0, 0);

                sendVirtualStickDataTimer.schedule(task, 0);
                sendVirtualStickDataTimer.schedule(task1, 700);
    ...
    private class VirtualStickDataManual extends TimerTask {
        float r;
        float p;
        float y;
        float thr;

        VirtualStickDataManual(float r, float p, float y, float thr) {
            this.r = r;
            this.p = p;
            this.y = y;
            this.thr = thr;
        ...
        public void run() {
            if (flightController != null) {
                //接口写反了，setPitch()应该传入roll值，setRoll()应该传入pitch值
                //the value is reverse in setpitch and in setroll, so when difining the value reverse them
                flightController.sendVirtualStickFlightControlData(new FlightControlData(r, p, y, thr), new CommonCallbacks.CompletionCallback() {
                    @Override
                    public void onResult(DJIError djiError) {
                        if (djiError != null) {
                            ToastUtils.setResultToToast(djiError.getDescription());
